...
% \makeatletter es necesario porque @ no es letra en código de usuario
\makeatletter
\AtBeginDocument{%
...
Para el desarrollo del proyecto se siguió el ciclo tradicional de aprendizaje automático: entendimiento del problema, recopilación y preparación de los datos, diseño y entrenamiento del modelo, evaluación sobre un conjunto de prueba independiente, y análisis de los resultados obtenidos.

\subsection{Datos}

Se utiliza el conjunto BBC News~\cite{c4}, compuesto por 2225 artículos periodísticos en cinco categorías: \textit{business} (510), \textit{entertainment} (386), \textit{politics} (417), \textit{sport} (511) y \textit{tech} (401). Cada entrada contiene el titular y el cuerpo del texto como cadena plana. El conjunto no presenta valores nulos.
...
El modelo se organiza en dos bloques, siguiendo el esquema de clasificación de secuencias de BERT~\cite{c2}. El primer bloque es DistilBERT~\cite{c5}, que procesa la secuencia mediante seis capas de atención multi-cabeza y produce una representación contextual para cada posición. Cada bloque transformer incluye una capa de atención con proyecciones lineal $Q$, $K$ y $V$ de dimensión 768, normalización de capa, una red de avance posición a posición con activación GELU (768\,$\to$\,3072\,$\to$\,768) y normalización de salida. La representación del token \texttt{[CLS]} se extrae como vector de 768 dimensiones. El segundo bloque es una cabeza de clasificación compuesta por una capa lineal intermedia (768\,$\to$\,768) seguida de una capa de salida (768\,$\to$\,5) con softmax. El modelo tiene aproximadamente 67 millones de parámetros. La Fig.~1 muestra la arquitectura completa.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.98\linewidth]{model_architecture.png}
    \caption{Arquitectura del modelo obtenida del notebook. El bloque DistilBERT (6 capas transformer) produce la representación \texttt{[CLS]}, que se proyecta sobre 5 clases mediante la cabeza de clasificación.}
\end{figure}
...
Los resultados del modelo entrenado se interpretan desde dos perspectivas, cuantitativa y cualitativa, siendo este análisis el insumo para plantear la dicusión acerca del proyecto.

\subsection{Resultados cuantitativos}

La Tabla~1 presenta los resultados sobre el conjunto de prueba (334 artículos). El modelo alcanza una exactitud global de \textbf{98.20\%} (328 de 334 artículos correctamente clasificados) y un F1-score macro de 0.982.
...
business           & 0.9737 & 0.9610 & 0.9673 & 77 \\ \hline
entertainment      & 0.9831 & 1.0000 & 0.9915 & 58 \\ \hline
politics           & 0.9672 & 0.9516 & 0.9593 & 62 \\ \hline
sport              & 1.0000 & 1.0000 & 1.0000 & 77 \\ \hline
tech               & 0.9836 & 1.0000 & 0.9917 & 60 \\ \hline
\textbf{macro avg} & \textbf{0.9815} & \textbf{0.9825} & \textbf{0.9820} & \textbf{334} \\ \hline
\end{tabular}}
\end{table}
