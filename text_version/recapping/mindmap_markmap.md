...
- Neocognitron (Fukushima)
  - Fukushima's 1980 architecture introduced alternating feature-extraction and subsampling layers, anticipating the conv–pool pattern of modern CNNs. It was the first system to learn position-invariant pattern recognition without explicit supervision.
...
- ImageNet moment (2012)
  - AlexNet's top-5 error rate of 15.3% on ImageNet — more than 10 points below the previous best — demonstrated that deep CNNs trained on GPUs vastly outperform hand-crafted feature pipelines. This result sparked the modern deep learning era and redirected the entire field.
...
- Binary cross-entropy
  - description
    - Specialised form of cross-entropy for two-class problems where the output is a single sigmoid probability. It penalises the model heavily when it assigns low probability to the correct binary label, and is also used as the discriminator loss in vanilla GANs.
  - Maths: L = −[y log(ŷ) + (1−y) log(1−ŷ)] — divergence between predicted probability and binary label
- KL Divergence
  - Acronym: KL = Kullback-Leibler (information-theoretic divergence measure)
    - Asymmetric measure of how much information is lost when approximating distribution P with Q. Used in VAE as the regularisation term keeping the latent space close to a Gaussian prior, and as the foundation of cross-entropy loss.
  - Maths: D_KL(P ‖ Q) = Σₓ P(x) log(P(x)/Q(x)) — how much P diverges from reference Q

## Convolutional Neural Networks (CNNs)
### Acronym: CNNs = Convolutional Neural Networks
- Deep neural networks that apply learnable filters across spatial dimensions to detect local patterns such as edges, textures, and shapes. Weight sharing across positions drastically reduces parameters and provides translation invariance, making CNNs the dominant architecture for image tasks.
### Core Components
- description
  - The architectural building blocks that give CNNs their power: convolution layers extract local features, pooling layers downsample spatially, and fully-connected layers combine those features into a final prediction. Each component serves a distinct role in transforming raw pixels into class scores.
...
- Fully-connected layers
  - Dense layers placed at the top of a CNN that take the flattened feature maps from the convolutional stack and combine them into a final output vector. They learn global relationships between all detected features and typically constitute the majority of a CNN's parameters.
- ReLU activations
  - Applied element-wise after each convolution (and often after each fully-connected layer) to introduce non-linearity. In CNNs, ReLU is preferred over sigmoid or tanh because it does not saturate for positive inputs and trains faster in deep stacks.
...
- AlexNet — 8 layers, ImageNet 2012
  - The first deep CNN to win ImageNet decisively, demonstrating that GPU-trained networks with ReLU activations and dropout regularisation vastly outperform hand-crafted features. Its five convolutional layers and three fully-connected layers became the template for the next generation of architectures.
- VGG — uniform 3×3 filters
  - Acronym: VGG = Visual Geometry Group (Oxford research group that designed the architecture)
    - Architecture from Oxford's VGG lab (2014) that showed depth could be increased by stacking only 3×3 filters uniformly. Straightforward to replicate and widely used as a feature extractor, though parameter-heavy due to large fully-connected layers.
- ResNet — skip connections
  - Acronym: ResNet = Residual Network — skip connections allow identity shortcuts through layers
...
- Inception — multi-scale filters
  - Applies filters of multiple kernel sizes (1×1, 3×3, 5×5) in parallel within the same layer, then concatenates their outputs. This multi-scale approach lets the network decide which spatial scale is most informative for each feature, achieving high accuracy with fewer parameters than VGG.

### Training for Vision
- description
  - Practical techniques applied during training to improve generalisation and stability of CNN models on visual data. Combining several of these methods is standard practice when working with limited or noisy image datasets.
- Data augmentation
...
- Batch normalization
  - Normalises each mini-batch's activations to zero mean and unit variance, then applies learnable scale and shift parameters. It stabilises and accelerates training, reduces sensitivity to initialisation, and acts as a mild regulariser, making it standard in nearly all modern CNNs.
- Dropout
...
### Applications
- description
  - CNNs are the backbone of most modern computer vision systems, from consumer apps to medical imaging and autonomous driving. Their translation invariance and hierarchical feature learning generalise across any task that involves local spatial patterns.
- Image classification
  - Assigns a single label (or ranked list of labels) to an entire input image. The canonical CNN task, popularised by ImageNet competitions, that drives most architecture research.
- Object detection
  - Locates and classifies multiple objects within an image, outputting bounding boxes alongside class labels. Frameworks such as YOLO and Faster R-CNN extend classification CNNs with region-proposal or anchor-based heads.
...
- Time series (1D conv)
  - Applies one-dimensional convolutional filters along the time axis to detect local temporal patterns in signals such as audio, sensor readings, or financial data. Faster to train than RNNs for many sequence tasks and easier to parallelise.

## Recurrent Neural Networks (RNNs)
### Acronym: RNNs = Recurrent Neural Networks
- Neural networks designed for sequential data where a hidden state carries information forward across timesteps using shared weights. They can model arbitrary-length sequences in theory, but struggle with long-range dependencies due to vanishing gradients — the problem [LSTM](#lstm-target) and GRU were designed to solve.
### Architectures
- description
  - The main RNN variants trade off between simplicity, parameter count, and ability to retain information over long sequences. Vanilla RNNs are the conceptual baseline; [LSTM](#lstm-target) and GRU add gating mechanisms that give the network explicit control over what to remember.
- Vanilla RNN
...
- GRU — update / reset gates
  - Acronym: GRU = Gated Recurrent Unit — streamlined [LSTM](#lstm-target) with fewer parameters
...
- Recurrent networks suffer from two fundamental optimisation problems that arise from multiplying the same weight matrix repeatedly across timesteps. Both problems make it hard to learn dependencies spanning more than a few dozen steps in vanilla RNNs.
#### Vanishing Gradient
- Gradients shrink through time
- Solutions: [LSTM](#lstm-target)/GRU gating, layer norm
#### Exploding Gradient
...
### Applications
- description
  - RNNs and their gated variants are applied wherever the input is a sequence of variable length or where temporal order carries meaning. They were the dominant sequence-modelling architecture before Transformers.
- Natural Language Processing
  - RNNs were the first effective neural approach to NLP tasks such as language modelling, machine translation, and sentiment analysis, processing text one token at a time while maintaining a context state.
...
- Speech recognition
  - Converts a variable-length audio sequence into a word sequence by modelling acoustic features across time. Bidirectional [LSTM](#lstm-target)s are particularly effective because each output token benefits from both past and future acoustic context.
- Anomaly detection
...
- The field of AI concerned with enabling computers to understand, generate, and manipulate human language. Modern NLP is dominated by pre-trained Transformer models fine-tuned on downstream tasks such as classification, translation, summarisation, and question answering.
#### Architecture
- description
...
- Encoder — bidirectional context
  - Processes the entire input sequence simultaneously with full bidirectional attention, so each token's representation is informed by all tokens to its left and right. Used in models like BERT for tasks requiring deep understanding of the input (classification, NER, question answering).
...
- Masked language modeling (BERT-style)
  - Acronym: BERT = Bidirectional Encoder Representations from Transformers
    - A large Transformer encoder pre-trained by predicting randomly masked tokens (MLM) using full bidirectional context. Released in 2018, it set state-of-the-art on almost every NLP benchmark and established the pre-train / fine-tune paradigm.
- Next-token prediction (GPT-style)
  - Acronym: GPT = Generative Pre-trained Transformer
...
- Pre-trained on ImageNet-21k
  - Large-scale pre-training on 21 million labelled images gives ViT the statistical diversity needed to learn general visual features before fine-tuning. Without this scale, ViT underperforms CNNs due to its lack of built-in inductive biases.
...
- Challenges: resolution sensitivity, scale
  - ViT's performance degrades when tested at resolutions different from those seen during pre-training because the number of patches changes and positional encodings must be interpolated. Additionally, ViT requires substantially more data and compute than comparably sized CNNs to achieve competitive results.
...
- Acronym: VAE = Variational Autoencoder — generative model with a regularised probabilistic latent space
  - An encoder maps inputs to a probability distribution over a latent space (rather than a fixed point), and a decoder samples from that distribution to reconstruct inputs. KL regularisation forces the latent space to be smooth and structured, enabling interpolation and controlled generation.
- Encoder: Q_φ(Z|X) — posterior approximation
  - A neural network that takes an input x and outputs the parameters (mean and variance) of a Gaussian distribution over the latent variable z. Training it jointly with the decoder and KL penalty encourages the latent distribution to stay close to a standard Gaussian prior.
...
- ELBO loss: reconstruction + KL term
  - Acronym: ELBO = Evidence Lower BOund ; KL = Kullback-Leibler divergence
    - The ELBO is what the VAE actually optimises — a lower bound on the log-likelihood of the data. It balances a reconstruction term (how well the decoder reproduces the input) against a KL penalty (how close the inferred latent distribution is to the Gaussian prior).
...
- Discriminator — real vs. fake classifier
  - A binary classifier trained to distinguish real training samples from samples produced by the generator. Provides the adversarial signal that drives the generator toward the true data distribution; in WGAN it is replaced by a critic without a sigmoid output.
...
#### Loss Functions
- description
  - The choice of loss function has a major impact on GAN training stability and sample quality. The original JS divergence suffers from vanishing gradients when distributions are disjoint; WGAN's Wasserstein distance and hinge loss are more robust alternatives.
- Jensen-Shannon Divergence (original)
  - description
    - Symmetric, bounded divergence measure used as the original GAN objective; minimising it is equivalent to the generator matching the real data distribution. In practice, training often collapses or oscillates because JS divergence saturates when the real and generated distributions have little overlap.
...
- Wasserstein distance (WGAN — more stable)
  - Acronym: WGAN = Wasserstein Generative Adversarial Network
    - A GAN variant that replaces the JS divergence with the Wasserstein (earth-mover) distance as the training objective. Meaningful gradients even when real and generated distributions have disjoint support make training more stable and less prone to mode collapse.
  - Maths: W(P,Q) = inf_{γ∈Π(P,Q)} 𝔼[‖x−y‖] — earth-mover distance, stable gradients even with disjoint support
- Hinge loss
  - Variant of the GAN objective adapted from support vector machines that clips the discriminator's output using a margin, preventing it from providing excessively large gradients. Widely adopted in high-resolution image synthesis models (BigGAN, StyleGAN) for its training stability.
#### Challenges
- Mode collapse
...
#### Applications
- description
  - GANs have become the workhorse for high-fidelity image synthesis tasks where perceptual sharpness is prioritised over likelihood. Their adversarial training produces crisp, photorealistic outputs that VAEs and early diffusion models struggled to match.
- Image synthesis
...
- Style transfer
  - Maps the visual style (texture, colour palette) of one image onto the content of another by separating and recombining feature statistics in a CNN or generator network.
- Super-resolution
...
- Video generation
  - Extends image GANs with temporal consistency constraints to synthesise coherent video sequences, enabling applications in film production, simulation, and data augmentation.

### Diffusion Models (DDPM)
#### Acronym: DDPM = Denoising Diffusion Probabilistic Models
...
#### Applications
- description
  - Diffusion models have become the state-of-the-art approach for high-quality image, audio, and video generation, surpassing GANs on perceptual metrics while being more stable to train.
- DALL-E 2
...
- Stable Diffusion
  - Runs the diffusion process in the latent space of a VAE rather than pixel space, dramatically reducing compute while maintaining image quality; open-source and widely deployed.
- Midjourney
...
### Acronym: LLMs = Large Language Models
...
### GPT Family (OpenAI)
- Acronym: GPT = Generative Pre-trained Transformer
  - OpenAI's decoder-only Transformer series scaled from GPT-1 (2018) through GPT-4 (2023). Each generation trained on larger corpora with more parameters; GPT-3 demonstrated few-shot learning and GPT-4 added multimodal inputs and substantially improved reasoning.
...
- GPT-1 → GPT-2 → GPT-3 → GPT-4
  - A scaling progression from 117M parameters (GPT-1) to an estimated trillion+ parameters (GPT-4), with each version demonstrating qualitatively new capabilities. GPT-2 showed fluent long-form text generation; GPT-3 introduced few-shot prompting; GPT-4 added multimodal reasoning.
- GPT-4: multimodal (text + image)
  - The first GPT model to accept image inputs alongside text, enabling tasks such as image captioning, chart interpretation, and visual question answering. Its vision encoder is integrated with the language decoder through cross-attention or prefix conditioning.
- ChatGPT: RLHF fine-tuned
  - Acronym: RLHF = Reinforcement Learning from Human Feedback — aligns model outputs to human preferences via reward modelling
    - A fine-tuning pipeline that collects human preference rankings over model outputs, trains a reward model from those rankings, then optimises the LLM against the reward model using policy-gradient RL (typically PPO). RLHF is the key step that turns a raw pre-trained model into a safe, helpful assistant.

### LLaMA (Meta)
- Acronym: LLaMA = Large Language Model Meta AI — Meta's efficient open-weights model family
  - Meta's series of decoder-only Transformers released with open weights, achieving strong performance at smaller scales through careful data curation and training efficiency improvements. Open weights democratised LLM research by enabling fine-tuning outside large compute clusters.
- Efficient decoder-only Transformer
  - LLaMA applies several architectural improvements over vanilla GPT — including pre-normalisation, SwiGLU activations, and rotary positional embeddings — to achieve better performance per parameter and more stable training at scale.
- Smaller size, competitive performance
  - LLaMA-2 13B matches or exceeds GPT-3 175B on many benchmarks by training on more tokens with higher-quality data. This shows that data quality and training efficiency matter as much as raw parameter count.
- Open weights for research
  - Unlike GPT-4, LLaMA weights are publicly released, enabling academic labs and independent researchers to study, fine-tune, and build upon the model without API access or commercial licensing costs.
...
- Text + image inputs
  - Combines visual features (from a CNN or ViT patch encoder) with text token embeddings in a unified sequence, allowing the model to answer questions about images, generate image captions, or produce images from text prompts.
...
- SGD (Stochastic Gradient Descent)
  - Acronym: SGD = Stochastic Gradient Descent — updates weights using mini-batch gradient estimates
...
- Adam / AdamW
  - Acronym: Adam = Adaptive Moment Estimation ; AdamW adds decoupled weight decay
...
- description
  - The practice of re-using representations learned on a large source task to bootstrap learning on a smaller target task. Drastically reduces the labelled data and compute required for each new application, and is the dominant training strategy across vision and NLP.
...
- Hugging Face Transformers
  - Open-source library providing thousands of pre-trained Transformer models (BERT, GPT, T5, ViT, Whisper, etc.) with a unified API for fine-tuning, inference, and dataset handling. Has become the de facto standard hub for sharing and deploying NLP and multimodal models.
...
- σ(x) = 1 / (1 + e⁻ˣ)
- description
  - Squashes any real value into (0, 1) — used for binary outputs and [LSTM](#lstm-target)/GRU gates
...
#### Chain Rule (Backprop)
- Acronym: Backprop = Backpropagation
- ∂L/∂w = (∂L/∂a) · (∂a/∂z) · (∂z/∂w)
- description
...
#### Adam Optimizer
- Acronym: Adam = Adaptive Moment Estimation
...
- Acronym: ELBO = Evidence Lower BOund
...
- Image classification pipeline
  - End-to-end workflow including data preprocessing, batching, model training with cross-entropy loss and SGD/Adam, learning rate scheduling, and computing accuracy metrics on a held-out test set.
...
- Movie/sentiment analysis
  - Trains an [LSTM](#lstm-target) or GRU to classify the sentiment of movie reviews from text, requiring the model to aggregate sentiment signals across variable-length sequences. Introduces tokenisation, embedding layers, and sequence padding.

### Miniproyecto 3 — Transformers NLP
- Project fine-tuning a pre-trained BERT-style Transformer on a text classification task, demonstrating the pre-train / fine-tune paradigm in practice. Covers the Hugging Face Transformers API, tokenisation, and evaluation metrics for NLP.
...
- Capstone project integrating multiple generative modelling techniques (VAE, GAN, or diffusion) to tackle a multimodal generation task. Emphasises understanding trade-offs between generation quality, diversity, and training stability.
...
- Lab RNN (m4) — time series
  - Acronym: RNN = Recurrent Neural Network
    - Neural networks that process sequential inputs by maintaining a hidden state, applicable to any time-ordered data. Vanilla RNNs, [LSTM](#lstm-target)s, and GRUs are the main variants covered in this lab.
...
- Lab ViT Part 1 & 2 (m6) — vision
  - Acronym: ViT = Vision Transformer
...
- Lab GPT (m8) — LLMs
  - Acronym: GPT = Generative Pre-trained Transformer ; LLMs = Large Language Models
    - Decoder-only Transformers trained on next-token prediction, scaled to billions of parameters to become LLMs. This lab explores causal language modelling and text generation with a GPT-style architecture.
